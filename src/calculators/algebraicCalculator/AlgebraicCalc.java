...
public class AlgebraicCalc extends AdvCalcFrame{

	private AlgebraicCalcGUI acGUI;
	
	private AlgebraicCalcEval evaluator; 
	...
	public AlgebraicCalc() {
    	...
    	ans = "";
		initialize();
		super.selMode[0].setSelected(true);
	}

	private void initialize() {
		
		acGUI = new AlgebraicCalcGUI();
		evaluator = new AlgebraicCalcEval();
		jBNumerical = acGUI.jBNumerical;					
		jBPoint = acGUI.jBPoint;							
		jBOperator = acGUI.jBOperator;					
		jBFunction = acGUI.jBFunction;		
		jBShift = acGUI.jBShift;							
		jBMemory = acGUI.jBMemory;						
		jBBracket = acGUI.jBBracket;						
		jBClear = acGUI.jBClear; 

		jBComma = acGUI.jBComma;
		jBVariable = acGUI.jBVariable;
		jTextFieldSelect = jTextDisplay;
		jVariableTextField = acGUI.jVariableText;
	
		for(int i=0; i<4; i++) {
    		jVariableTextField[i].setText("");
    	}

        jLabelDisplay = acGUI.jLabelDisplay;
        jTextDisplay = acGUI.jTextDisplay;
	...
	private void jBOperator(ActionEvent evt){
		...
		if(evt.getActionCommand().equals("=")){
			...
			if(jTextDisplay.getText().equals("")) {
				...
				String result = evaluator.evaluate(jTextDisplay.getText(),jVariableTextField[0].getText(),jVariableTextField[1].getText(),jVariableTextField[2].getText(),jVariableTextField[3].getText());
