...
public class IntegralCalc extends AdvCalcFrame{

	private IntegralCalcGUI icGUI;
	
	private IntegralCalcEval evaluator= new IntegralCalcEval();
	...
	private JTextField[] jLimitTextField;
	...
	public IntegralCalc() {
    	...
    	ans="";
		initialize();
		super.selMode[2].setSelected(true);
	}

	private void initialize() {
		icGUI = new IntegralCalcGUI();

		jBNumerical = icGUI.jBNumerical;					
		jBPoint = icGUI.jBPoint;							
		jBOperator = icGUI.jBOperator;					
		jBFunction = icGUI.jBFunction;		
		jBShift = icGUI.jBShift;							
		jBMemory = icGUI.jBMemory;						
		jBBracket = icGUI.jBBracket;						
		jBClear = icGUI.jBClear; 
		///////
		jBComma = icGUI.jBComma;
		jBVariable = icGUI.jBVariable;
		jTextFieldSelect=jTextDisplay;
		jVariableTextField=icGUI.jVariableText;
		jLimitTextField=icGUI.jLimitTextField;
		
		for(int i=0; i<2; i++) {
			jLimitTextField[i].setText("");
		}
        ...
        jLabelDisplay = icGUI.jLabelDisplay;
        jTextDisplay = icGUI.jTextDisplay;
		...
		jLimitTextField[0].addMouseListener(new MouseAdapter(){
	        public void mouseClicked(MouseEvent e){
	            jTextFieldSelect=jLimitTextField[0];
	        }
	    });
		jLimitTextField[1].addMouseListener(new MouseAdapter(){
	        public void mouseClicked(MouseEvent e){
	            jTextFieldSelect=jLimitTextField[1];
	        }
	    });
	...
	private void jBOperator(ActionEvent evt){
		...
		if(evt.getActionCommand().equals("=")){
			...
			if(jLimitTextField[0].getText().equals("")) {
				jLimitTextField[0].setText("");
			}
			if(jLimitTextField[1].getText().equals("")) {
				jLimitTextField[1].setText("");
			}
			if(jTextDisplay.getText().equals("")) {
				jLabelDisplay.setText("0.0000000");
			}
			else {
				String a1 = jTextDisplay.getText();
				String a2 = jVariableTextField[0].getText();
				String a3 = jVariableTextField[1].getText();
				String a4 = jVariableTextField[2].getText();
				String a5 = jVariableTextField[3].getText();
				String a6 = jLimitTextField[0].getText();
				String a7 = jLimitTextField[1].getText();
				String result = evaluator.integrate(a1,a2,a3,a4,a5,a6,a7);
